...
public class DlgCariPeriksaRadiologi extends javax.swing.JDialog
{
    ...
    public DlgCariPeriksaRadiologi(java.awt.Frame parent, boolean modal)
    {
        ...
        Object[] row =
        {
            "No.Rawat", "Pasien", "Petugas", "Tgl.Periksa", "Jam Periksa", "Dokter Perujuk", "Penanggung Jawab"
        };
    ...
    private void initComponents() {
        ...
        MnBarcode.setText("Label Radiologi");
...
private void BtnHapusActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnHapusActionPerformed
    ...
    else if (!(Kd2.getText().trim().equals("")))
    {
        ...
        else if (Sequel.cariRegistrasi(tbDokter.getValueAt(tbDokter.getSelectedRow(), 0).toString()) > 0)
        {
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        ...
        else
        {
            ...
            else if (Sequel.menyimpantf2("hasil_radiologi", "?,?,?,?", "Hasil Pemeriksaan", 4, new String[]
            {
    ...
    {//GEN-HEADEREND:event_MnBarcodeActionPerformed
        this.setCursor(Cursor.getPredefinedCursor(Cursor.WAIT_CURSOR));
        ...
        else if (!(Kd2.getText().trim().equals("")))
        {
            ...
            String q = "SELECT reg_periksa.no_rkm_medis as no_rm, pasien.nm_pasien "
                    + "FROM reg_periksa "
                    + "JOIN pasien ON pasien.no_rkm_medis = reg_periksa.no_rkm_medis "
                    + "WHERE reg_periksa.no_rawat = '" + Kd2.getText() + "'";

            List<String[]> res = Sequel.select(q);

            if (res.isEmpty())
            {
                JOptionPane.showMessageDialog(null,
                        "Maaf, Gagal mencteak. Data tidak ditemukan!!!");
            }

            param.put("nama", res.get(0)[1]);
            param.put("alamat", Sequel.cariIsi("select date_format(tgl_lahir,'%d/%m/%Y') from pasien where no_rkm_medis=?", res.get(0)[0]));
            param.put("norm", res.get(0)[0]);
            ...
            Valid.MyReport("rptBarcodeRawat.jrxml", "report", "::[ Barcode No.Rawat ]::",
                    "select reg_periksa.no_rawat from reg_periksa where no_rawat='" + Kd2.getText() + "'", param);
        }
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                while (rs2.next())
                {
                    ...
                    tabMode.addRow(new Object[]
                    {
                        "", "", rs2.getString("kd_jenis_prw"), rs2.getString("nm_perawatan"), Valid.SetAngka(rs2.getDouble("biaya")), "", ""
                    });
                }
                tabMode.addRow(new Object[]
                {
                    "", "", "Kode BHP", "Nama BHP", "Satuan", "Jumlah", ""
                });
                ...
                while (rs3.next())
                {
                    tabMode.addRow(new Object[]
                    {
                        "", "", rs3.getString("kode_brng"), rs3.getString("nama_brng"), rs3.getString("kode_sat"), rs3.getString("jumlah"), ""
                    });
                }
                ...
                while (rs5.next())
                {
                    tabMode.addRow(new Object[]
                    {
                        "", "", "Hasil Pemeriksaan :", rs5.getString("hasil"), "", "", ""
                    });
                }
            }
            if (ttl > 0)
