...
import simrskhanza.cari.DlgCariGroupTindRanap;
...
public final class DlgCariPerawatanRanap extends javax.swing.JDialog
{
    ...
    private String pilihtable = "", kd_pj = "", kd_bangsal = "", kdKelas = "", ruang_ranap = "Yes", cara_bayar_ranap = "Yes";
    ...
    public DlgCariPerawatanRanap(java.awt.Frame parent, boolean modal)
    {
        ...
        // Kalo nama group berubah refresh table ==
        txtNamaGroup.addTextChangedListener(t -> tampil());
    ...
    private void initComponents()
    {
        ...
        cmbCariGroup = new widget.Button();
        ...
        DTPTgl.setModel(new javax.swing.DefaultComboBoxModel(new String[] { "14-05-2018" }));
        ...
        cmbCariGroup.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/190.png"))); // NOI18N
        cmbCariGroup.setMnemonic('4');
        cmbCariGroup.setToolTipText("ALt+4");
        cmbCariGroup.setName("cmbCariGroup"); // NOI18N
        cmbCariGroup.setPreferredSize(new java.awt.Dimension(28, 23));
        cmbCariGroup.addActionListener(new java.awt.event.ActionListener()
        {
            public void actionPerformed(java.awt.event.ActionEvent evt)
            {
                cmbCariGroupActionPerformed(evt);
            }
        });
        FormInput.add(cmbCariGroup);
        cmbCariGroup.setBounds(910, 40, 28, 23);

        txtNamaGroup.setEditable(false);
        txtNamaGroup.setHighlighter(null);
        txtNamaGroup.setName("txtNamaGroup"); // NOI18N
        txtNamaGroup.setPreferredSize(new java.awt.Dimension(400, 23));
        FormInput.add(txtNamaGroup);
        txtNamaGroup.setBounds(595, 40, 310, 23);

        txtKdGroup.setHighlighter(null);
        txtKdGroup.setName("txtKdGroup"); // NOI18N
        txtKdGroup.setPreferredSize(new java.awt.Dimension(120, 23));
        ...
        FormInput.add(txtKdGroup);
        txtKdGroup.setBounds(520, 40, 70, 23);

        jLabel4.setText("Group :");
        jLabel4.setName("jLabel4"); // NOI18N
        FormInput.add(jLabel4);
        jLabel4.setBounds(466, 40, 50, 23);
...
private void ppPetugasActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppPetugasActionPerformed
    ...
    private void cmbCariGroupActionPerformed(java.awt.event.ActionEvent evt)//GEN-FIRST:event_cmbCariGroupActionPerformed
    {//GEN-HEADEREND:event_cmbCariGroupActionPerformed
        DlgCariGroupTindRanap d = new DlgCariGroupTindRanap(null, true);
        ...
        if (d.RESULT)
        {
            txtKdGroup.setText(d.DATA[0]);
            txtNamaGroup.setText(d.DATA[1]);
        }
    }//GEN-LAST:event_cmbCariGroupActionPerformed

    private void txtKdGroupKeyPressed(java.awt.event.KeyEvent evt)//GEN-FIRST:event_txtKdGroupKeyPressed
    {//GEN-HEADEREND:event_txtKdGroupKeyPressed
        if (evt.getKeyCode() == KeyEvent.VK_ENTER)
        {
            Sequel.cariIsi("SELECT nm_group FROM hrj_group_ranap WHERE kd_group = ?", txtNamaGroup, txtKdGroup.getText());

            if (txtNamaGroup.getText().isEmpty())
            {
                txtKdGroup.setText("");
            }
        }
    ...
    private widget.Button cmbCariGroup;
    ...
    public void tampil()
    {
        try
        {
            ...
            // Yang dipakai yg ini, yg lain abaikan saja ====
            pscari4 = koneksi.prepareStatement("select jns_perawatan_inap.kd_jenis_prw,jns_perawatan_inap.nm_perawatan, jns_perawatan_inap.kd_kategori,"
                    + "jns_perawatan_inap.total_byrdr,jns_perawatan_inap.total_byrpr,jns_perawatan_inap.total_byrdrpr,jns_perawatan_inap.bhp,jns_perawatan_inap.material,jns_perawatan_inap.kso,jns_perawatan_inap.menejemen,"
                    + "jns_perawatan_inap.tarif_tindakandr,jns_perawatan_inap.tarif_tindakanpr "
                    + "from jns_perawatan_inap "
                    + "where kd_pj = ? AND kd_bangsal = ? AND kd_kelas = ? AND kd_kategori = ? AND jns_perawatan_inap.status='1' "
                    + "AND (jns_perawatan_inap.kd_jenis_prw like ? or jns_perawatan_inap.nm_perawatan like ?) "
            ...
            try
            {
                ...
                // Yang dipakai yang ini, yg lain abaikan saja =============
                else if (ruang_ranap.equals("No") && cara_bayar_ranap.equals("No"))
                {
                    pscari4.setString(1, kd_pj);
                    pscari4.setString(2, kd_bangsal);
                    pscari4.setString(3, kdKelas);
                    pscari4.setString(4, txtKdGroup.getText());
                    pscari4.setString(5, "%" + TCari.getText().trim() + "%");
    ...
    public void setNoRm(String norwt, String pilihtable, Date tanggal, String jam, String menit, String detik, boolean status, String pasien)
    {
        ...
        // Mencari kdKelas
        this.kdKelas = Sequel.cariIsi(
                "select kamar.kd_kelas from bangsal inner join kamar inner join kamar_inap "
                + "on bangsal.kd_bangsal=kamar.kd_bangsal and kamar.kd_kamar=kamar_inap.kd_kamar "
                + "where no_rawat=? and stts_pulang='-' order by STR_TO_DATE(concat(kamar_inap.tgl_masuk,' ',jam_masuk),'%Y-%m-%d %H:%i:%s') desc limit 1", TNoRw.getText());
        ...
        try
        {
            ...
            try
            {
                ...
                // Tarif sudah diatur berdasarkan kelas, cara bayar, dll
                if (true)
                {
                    ruang_ranap = "No";
                    cara_bayar_ranap = "No";
                }
                else if (rstarif.next())
                {
